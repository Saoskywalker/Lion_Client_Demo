...
/*******发送帧头*********/
static __inline dwSendStart(void)
{
	MTF_Com_sendHead();
}
/*******发送帧尾*********/
static __inline dwSendOver(void)
{
	MTF_Com_sendTail();
}
/***发送帧用户数据***/
...
/*屏幕准备完成和通信检测*/
#include "delay.h"
#define BEEP_PIN PAout(4)
void screenCheck(void)
{
	u8 i;
	//Check communicate with DW Display, continue 18s
	for (i = 0; i < 90; i++)
	{
		dwSendStart();
		dwSendBuf(0x00);  // 发送握手指令
		dwSendOver();
		delay_ms(5);
		if (dwQueryCmd() == 0x00)
		{ // 握手成功
